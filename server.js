...
async function userView(id){return (await q("SELECT id,username,role,coins,played_coins,total_opened,banned,last_daily_at,created_at FROM users WHERE id=$1",[id])).rows[0]}
...
const rewards=[
 {name:"500K Yang",icon:"💰",type:"yang",amount:500000,weight:55},
 {name:"1M Yang",icon:"💎",type:"yang",amount:1000000,weight:25},
 {name:"5M Yang",icon:"👑",type:"yang",amount:5000000,weight:8},
 {name:"10M Jackpot",icon:"🏆",type:"yang",amount:10000000,weight:2},
 {name:"Ritka PET",icon:"🐾",type:"item",amount:1,weight:5},
 {name:"500 Coin",icon:"🪙",type:"coin",amount:500,weight:5}
];
...
app.post("/api/open",auth,async(req,res)=>{
 ...
 try{
   ...
   const results=[],items={};let coinWin=0;
   for(let i=0;i<qty;i++){const r=pick();results.push(r);if(r.type==="coin")coinWin+=r.amount;if(r.type==="item")items[r.name]=(items[r.name]||0)+1}
   await client.query("UPDATE users SET coins=coins-$1+$2,played_coins=played_coins+$1,total_opened=total_opened+$3 WHERE id=$4",[cost,coinWin,qty,req.user.id]);
   ...
   res.json({user:await userView(req.user.id),results,cost});
...
app.get("/api/leaderboard",async(req,res)=>res.json({rows:(await q("SELECT username,played_coins,total_opened FROM users WHERE role='user' AND banned=FALSE ORDER BY played_coins DESC LIMIT 10")).rows}));
...
app.get("/api/admin/users",auth,admin,async(req,res)=>{
 ...
 const rows=s?(await q("SELECT id,username,role,coins,played_coins,total_opened,banned,created_at FROM users WHERE username ILIKE $1 ORDER BY id DESC",["%"+s+"%"])).rows:
 (await q("SELECT id,username,role,coins,played_coins,total_opened,banned,created_at FROM users ORDER BY id DESC LIMIT 300")).rows;
 res.json({rows});
});
